...
export default function CustomerProfilePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (cData) {
        ...
        type ComplaintRowMin = { id: string; [key: string]: unknown };
        ...
        const lTyped = (lData ?? []) as ComplaintLogRowMin[];
        const complaintsWithLogs = cTyped.map((c) => ({
          ...c,
          logs: lTyped.filter((l) => l.complaint_id === c.id),
        }));
  ...
  const handleSaveMetadata = async () => {
    ...
    const { error } = await supabase.from('turath_masr_customers').upsert({
      phone,
    ...
    <AppLayout currentPath="/crm">
      ...
      <div className="space-y-8 fade-in pb-20 pt-2" dir="rtl">
        ...
        <div className="relative overflow-hidden bg-white border-2 border-gray-50 rounded-[3rem] p-8 lg:p-12 shadow-sm">
          ...
          <div
            className={`absolute top-0 right-0 w-full h-32 bg-gradient-to-l opacity-10 ${SEGMENT_CLASSES[metadata.segment]}`}
          />
          ...
          <div className="flex flex-col lg:flex-row gap-10 relative z-10 text-right">
            ...
            <div className="flex flex-col items-center lg:items-start gap-6 text-center lg:text-right">
              <div
                className={`w-32 h-32 rounded-[2.5rem] bg-gradient-to-br flex items-center justify-center text-5xl font-black shadow-2xl shadow-emerald-200/20 text-white ${SEGMENT_CLASSES[metadata.segment]}`}
              >
              ...
              {[
                {
                  ...
                  value: `${financialStats.totalSpent.toLocaleString()} ج.م`,
                  icon: <DollarSign className="text-emerald-500" />,
                  bg: 'bg-emerald-50/50',
                },
                {
                  label: 'عدد الطلبات',
                  value: financialStats.orderCount,
                  icon: <ShoppingBag className="text-blue-500" />,
                  bg: 'bg-blue-50/50',
                },
                {
                  label: 'نسبة النجاح',
                  value: `${financialStats.successRate}%`,
                  icon: <ShieldCheck className="text-purple-500" />,
                  bg: 'bg-purple-50/50',
                },
                {
                  label: 'متوسط الأوردر',
                  value: `${financialStats.avgOrderValue} ج.م`,
                ...
                <div
                  key={i}
                  className={`${stat.bg} p-6 rounded-[2rem] border border-transparent hover:border-white hover:shadow-xl transition-all group`}
                >
            ...
            <div className="bg-white border-2 border-gray-50 rounded-[2.5rem] p-8 shadow-sm">
              ...
              <div className="space-y-4">
                {orders.map((o) => (
                  <div
                    key={o.id}
                    className="group bg-gray-50/30 hover:bg-white border hover:border-blue-100 hover:shadow-lg rounded-[1.5rem] p-5 transition-all flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-medium italic mt-0.5 line-clamp-1 max-w-[200px]">
                          {o.products}
                        </p>
                      </div>
                    </div>
                    <div className="text-left">
                      <span
                        className={`text-[10px] font-black px-3 py-1 rounded-full border uppercase ${STATUS_COLORS[o.status] || 'bg-gray-50'}`}
                      >
                        {o.status}
                      </span>
                      <p className="text-xs font-black text-gray-900 font-mono mt-1">
                        {Number(o.total).toLocaleString()} ج.م
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white border-2 border-gray-50 rounded-[2.5rem] p-8 shadow-sm">
              ...
              <div className="space-y-6">
                {complaints.map((c) => (
                  <div
                    key={c.id}
                    className="relative pl-8 border-l-2 border-gray-50 last:border-0 pb-6"
                  >
                    ...
                    <div className="bg-gray-50/50 rounded-2xl p-5 hover:bg-orange-50/30 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`text-[9px] font-black px-2 py-0.5 rounded-lg border ${c.status === 'resolved' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}
                        >
                          {c.status}
                        </span>
                      </div>
                              ...
                              {log.old_status !== log.new_status && (
                                <div className="flex items-center gap-1.5 mt-1 border-t border-gray-50 pt-1">
                                  <span className="text-[8px] font-bold text-emerald-600">
                                    تغيرت الحالة: {log.old_status} → {log.new_status}
                                  </span>
                                </div>
                              )}
                            </div>
