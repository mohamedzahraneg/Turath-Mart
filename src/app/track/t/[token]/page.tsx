...
// Phase Order-Documents-1 — shared pure HTML builders for the
// professional invoice + warranty certificate. Same helper used by
// the admin OrderDetailModal print path. Customer-side calls always
// pass `audience: 'customer'` so staff-only fields (phone2, delegate
// name, internal-only data) are dropped at the template boundary
// even when the RPC inadvertently surfaces them.
import {
  buildInvoiceHtml,
...
// Phase Order-Documents-1 — thin wrapper around the shared
// `buildInvoiceHtml` helper. The customer audience drops admin-only
// fields (phone2 / delegate name) at the template boundary even if
// the RPC happens to surface them. Tracking link uses the
// unguessable /track/t/<token> URL so reshares stay on the
// token-keyed path.
function generateInvoiceHTML(order: TrackingOrder, token: string): string {
  ...
  const trackingLink = `${origin}/track/t/${token}`;
  ...
  const input: InvoiceInput = {
    ...
    createdAt: order.createdAt || `${order.date} ${order.time}`,
    ...
    pricing: {
      ...
      extraShippingFee:
        order.extraShippingFee && order.extraShippingFee > 0 ? order.extraShippingFee : null,
      discount:
        checkout?.discount?.amount && checkout.discount.amount > 0
          ? checkout.discount.amount
          : null,
      total: order.total,
      paid: checkout?.payment?.paid_amount ?? null,
      remaining: checkout?.payment?.remaining_amount ?? null,
      paymentMethod: checkout?.payment?.method ?? null,
      paymentStatus: checkout?.payment?.status ?? null,
    },
    schedule: order.scheduledDelivery
      ? {
          date: order.scheduledDelivery.date,
          formatted: `${formatScheduleDateShortAr(order.scheduledDelivery.date)} — ${formatTime12hAr(order.scheduledDelivery.from)} إلى ${formatTime12hAr(order.scheduledDelivery.to)}`,
        }
      : null,
...
// Phase Order-Documents-1 — thin wrapper. Returns empty string when
// the order has no warranty (handler should not call this in that
// case; the JSX already hides the button when `hasWarranty` is
// falsy, but the helper is defensive).
