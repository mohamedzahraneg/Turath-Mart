...
CREATE TABLE IF NOT EXISTS public.zahranship_crm_complaints (
...
CREATE TABLE IF NOT EXISTS public.zahranship_crm_chat (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    customer_phone TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.zahranship_settings (
    key TEXT PRIMARY KEY,
    value JSONB NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

ALTER TABLE public.zahranship_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow authenticated users to read/write settings"
    ON public.zahranship_settings
    FOR ALL TO authenticated
    USING (true);

-- Enable Real-time for settings
ALTER PUBLICATION supabase_realtime ADD TABLE zahranship_settings;
