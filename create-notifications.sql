...
CREATE TABLE IF NOT EXISTS public.turath_masr_notifications (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    type TEXT NOT NULL, -- 'new_order', 'status_change', 'inventory_alert', 'whatsapp_sent'
    title TEXT NOT NULL,
    message TEXT NOT NULL,
    order_id TEXT, -- References turath_masr_orders.id
    order_num TEXT,
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE turath_masr_notifications;
    ELSE
        CREATE PUBLICATION supabase_realtime FOR TABLE turath_masr_notifications;
    END IF;
END $$;
